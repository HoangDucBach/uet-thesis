...
public fun create_pool<TokenA, TokenB>(
    ...
    let initial_liquidity = math_utils::sqrt(amount_a * amount_b);
...
public fun swap_b_to_a<TokenA, TokenB>(
    ...
    let amount_in_with_fee = amount_in * (10000 - pool.fee_rate);
    let numerator = amount_in_with_fee * reserve_a_val;
    let denominator = reserve_b_val * 10000 + amount_in_with_fee;
    let amount_out = numerator / denominator;
...
public fun add_liquidity<TokenA, TokenB>(
    ...
    let liquidity = if (pool.lp_supply == 0) {
        math_utils::sqrt(amount_a * amount_b)
    } else {
