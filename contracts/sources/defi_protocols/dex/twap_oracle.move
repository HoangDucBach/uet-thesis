...
module simulation::twap_oracle {
    ...
    public struct PriceObservation has store, copy, drop {
    ...
    public fun update_observation<TokenA, TokenB>(
        ...
        let (price_cum_a, price_cum_b) = if (vector::length(&oracle.observations) > 0) {
            let last_obs = vector::borrow(&oracle.observations,
    ...
    public fun get_twap<TokenA, TokenB>(
        ...
        // Check staleness
        assert!(
            current_time - latest_obs.timestamp < MAX_STALENESS_MS,
            E_STALE_PRICE
        );
    ...
    public fun get_spot_price<TokenA, TokenB>(
        oracle: &TWAPOracle<TokenA, TokenB>,
    ): (u64, u64) {
        let obs_count = vector::length(&oracle.observations);
    ...
    public fun get_observation_count<TokenA, TokenB>(
        oracle: &TWAPOracle<TokenA, TokenB>
    ): u64 {
    ...
    public fun get_window_size<TokenA, TokenB>(
        oracle: &TWAPOracle<TokenA, TokenB>
    ): u64 {
