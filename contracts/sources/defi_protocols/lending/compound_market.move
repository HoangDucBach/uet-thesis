...
module defi_protocol::compound_market {
    ...
    const E_MARKET_NOT_LISTED: u64 = 5;
    const E_BORROW_CAP_REACHED: u64 = 6;
    ...
    public fun withdraw<T>(
        ...
        let exchange_rate = get_exchange_rate(market);
        let underlying_amount = (c_tokens_to_burn as u128) * (exchange_rate as u128) / (PRECISION as u128);
    ...
    public fun borrow<Collateral, Debt>(
        ...
        let underlying_collateral = (position.c_token_balance as u128) * (exchange_rate as u128) / (PRECISION as u128);
        let underlying_collateral = (underlying_collateral as u64);

        let collateral_value = (underlying_collateral as u128) * (oracle_price as u128) / (PRECISION as u128);
        let collateral_value = (collateral_value as u64);

        // Calculate max borrow based on collateral factor
        let max_borrow = (collateral_value as u128) * (collateral_market.collateral_factor as u128) / (BPS_PRECISION as u128);
    ...
    public fun liquidate<Collateral, Debt>(
        ...
        let seize_value = (actual_repay as u128) * (BPS_PRECISION as u128 + LIQUIDATION_INCENTIVE as u128) / (BPS_PRECISION as u128);
        ...
        let underlying_to_transfer = (actual_seize_c_tokens as u128) * (exchange_rate as u128) / (PRECISION as u128);
        ...
        coin::from_balance(balance::split(&mut collateral_market.total_cash, underlying_to_transfer), ctx)
    }
    ...
    fun accrue_interest<T>(market: &mut Market<T>, clock: &Clock) {
        ...
        let interest_accumulated = (market.total_borrows as u128) * interest_factor / (PRECISION as u128);
        let interest_accumulated = (interest_accumulated as u64);

        // Calculate reserves
        let reserves_added = (interest_accumulated as u128) * (market.reserve_factor as u128) / (BPS_PRECISION as u128);
        let reserves_added = (reserves_added as u64);

        // Update state
        market.total_borrows = market.total_borrows + interest_accumulated;
        market.borrow_index = market.borrow_index + ((market.borrow_index as u128) * interest_factor / (PRECISION as u128) as u64);
    ...
    fun get_borrow_rate<T>(market: &Market<T>): u64 {
        ...
        if (utilization <= market.kink) {
            ...
            let rate = market.base_rate_per_second +
            ...
            let normal_rate = market.base_rate_per_second +
    ...
    public fun calculate_health_factor(
        ...
        let adjusted_collateral = (collateral_value as u128) * (liquidation_threshold as u128) / (BPS_PRECISION as u128);
    ...
    fun calculate_accrued_interest(
        ...
        let interest_factor = ((new_index - old_index) as u128) * (PRECISION as u128) / (old_index as u128);
