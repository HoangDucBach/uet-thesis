...
public fun mint_usdc(factory: &mut CoinFactory, amount: u64, ctx: &mut TxContext): Coin<USDC> {
    coin::mint(&mut factory.usdc_treasury, amount, ctx)
}

/// Mint USDT coins
public fun mint_usdt(factory: &mut CoinFactory, amount: u64, ctx: &mut TxContext): Coin<USDT> {
    coin::mint(&mut factory.usdt_treasury, amount, ctx)
}

/// Mint WETH coins
public fun mint_weth(factory: &mut CoinFactory, amount: u64, ctx: &mut TxContext): Coin<WETH> {
    coin::mint(&mut factory.weth_treasury, amount, ctx)
}

/// Mint BTC coins
public fun mint_btc(factory: &mut CoinFactory, amount: u64, ctx: &mut TxContext): Coin<BTC> {
