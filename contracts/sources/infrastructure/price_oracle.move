...
public fun get_price<T>(oracle: &PriceOracle, clock: &Clock): (u64, u64) {
    // (price, confidence)
    let token_type = type_name::with_defining_ids<T>();
...
public fun add_authorized_source(oracle: &mut PriceOracle, source: address, ctx: &TxContext) {
...
public fun remove_authorized_source(oracle: &mut PriceOracle, source: address, ctx: &TxContext) {
    assert!(tx_context::sender(ctx) == oracle.admin, E_NOT_AUTHORIZED);
