...
module simulation::flash_loan_tests {
    ...
    fun test_create_flash_loan_pool() {
        ...
        test::next_tx(&mut scenario, ALICE);
        {
            ...
            let initial_funds = coin_factory::mint_usdc(&mut factory, POOL_LIQUIDITY, test::ctx(&mut scenario));
    ...
    fun test_add_liquidity_to_pool() {
        ...
        test::next_tx(&mut scenario, BOB);
        {
            ...
            // Add more liquidity
            let additional = coin_factory::mint_usdc(&mut factory, POOL_LIQUIDITY, test::ctx(&mut scenario));
    ...
    fun test_pool_statistics() {
        ...
        {
            let pool = test::take_shared<FlashLoanPool<USDC>>(&scenario);
            let (balance, total_borrowed, loan_count, fee_rate) = flash_loan_pool::get_stats(&pool);
