...
#[test_only]
module defi_protocol::lending_tests {
    use sui::test_scenario::{Self as test, Scenario, next_tx, ctx};
    use sui::coin::{Self, Coin};
    use sui::test_utils;
    use sui::clock::{Self, Clock};

    use defi_protocol::compound_market::{Self, Market, Position};
    use defi_protocol::simple_dex::{Self, Pool};
    use defi_protocol::usdc::USDC;
    use defi_protocol::weth::WETH;
    use defi_protocol::coin_factory::{Self, CoinFactory};
    ...
    fun setup_markets(scenario: &mut Scenario): (address, address) {
        ...
        {
            let factory = test::take_shared<CoinFactory>(scenario);
            let clock = test::take_shared<Clock>(scenario);

            // Mint initial liquidity for USDC market
            let usdc_liquidity = coin_factory::mint_usdc(&mut factory, 1_000_000_000_000, ctx(scenario));

            let usdc_market = compound_market::create_market<USDC>(
                usdc_liquidity,
        ...
        next_tx(scenario, ADMIN);
        {
            ...
            let weth_liquidity = coin_factory::mint_weth(&mut factory, 100_000_000_000, ctx(scenario));

            let weth_market = compound_market::create_market<WETH>(
                weth_liquidity,
                @0x2,
                7500,
                1000,
        ...
        next_tx(scenario, ADMIN);
        {
            ...
            let pool = simple_dex::create_pool(weth, usdc, ctx(scenario));
    ...
    fun test_supply_and_withdraw() {
        ...
        {
            let factory = test::take_shared<CoinFactory>(scenario);
            let market = test::take_shared<Market<USDC>>(scenario);
            let clock = test::take_shared<Clock>(scenario);
        ...
        next_tx(scenario, ALICE);
        {
            let market = test::take_shared<Market<USDC>>(scenario);
            let clock = test::take_shared<Clock>(scenario);
            let position = test::take_from_sender<Position<USDC>>(scenario);
    ...
    fun test_borrow_and_repay() {
        ...
        {
            let factory = test::take_shared<CoinFactory>(scenario);
            let market = test::take_shared<Market<WETH>>(scenario);
            let clock = test::take_shared<Clock>(scenario);
        ...
        next_tx(scenario, ALICE);
        {
            let weth_market = test::take_shared<Market<WETH>>(scenario);
            let usdc_market = test::take_shared<Market<USDC>>(scenario);
            let pool = test::take_shared<Pool<WETH, USDC>>(scenario);
            let clock = test::take_shared<Clock>(scenario);
            let position = test::take_from_sender<Position<WETH>>(scenario);
    ...
    fun test_oracle_manipulation_attack() {
        ...
        next_tx(scenario, BOB);
        {
            ...
            let usdc_market = test::take_shared<Market<USDC>>(scenario);
            let pool = test::take_shared<Pool<WETH, USDC>>(scenario);
            let flash_pool = test::take_shared<flash_loan_pool::FlashLoanPool<USDC>>(scenario);
            let clock = test::take_shared<Clock>(scenario);

            // Step 1: Borrow flash loan (10M USDC)
            let (flash_loan_coins, flash_loan_receipt) = flash_loan_pool::borrow_flash_loan(
                &mut flash_pool,
                10_000_000_000_000,
                ctx(scenario)
            );
            ...
            let manipulation_amount = coin::split(&mut flash_loan_coins, 5_000_000_000_000, ctx(scenario));
            ...
            let position = compound_market::supply(&mut weth_market, weth_from_swap, &clock, ctx(scenario));
    ...
    fun test_liquidation() {
        ...
        next_tx(scenario, ALICE);
        {
            ...
            // Supply 1 WETH
            let weth = coin_factory::mint_weth(&mut factory, 1_000_000, ctx(scenario));
            let position = compound_market::supply(&mut weth_market, weth, &clock, ctx(scenario));
        ...
        next_tx(scenario, ADMIN);
        {
            ...
            let usdc = coin_factory::mint_usdc(&mut factory, 100_000_000_000, ctx(scenario));
            let weth_out = simple_dex::swap_b_to_a(&mut pool, usdc, 0, ctx(scenario));

            coin::burn_for_testing(weth_out);
            test::return_shared(factory);
            test::return_shared(pool);
        };
